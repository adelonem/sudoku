...
def _forcing_chains(state, upgrade):
    ...
    - If a branch leads to a contradiction, that candidate can be eliminated.
    - If all valid branches eliminate the same candidate elsewhere, eliminate it.
    - If all branches place the same digit in another cell, place it.
    """
    found = False
    ...
    for ci in range(81):
        ...
        valid = [s for s in branches if s is not None]
        invalid_idx = [i for i, s in enumerate(branches) if s is None]

        # Contradictory branches → eliminate those candidates
        for i in invalid_idx:
            if state.eliminate(ci, cands[i]):
                upgrade('diabolic')
                found = True
        ...
        for cj in range(81):
            ...
            for d in list(state.candidates[cj]):
                # Eliminated in every valid branch
                if all((s.grid[cj] != 0 and s.grid[cj] != d) or
                    ...
                    if state.eliminate(cj, d):
                        upgrade('diabolic')
                        found = True
                # Placed as d in every valid branch
...
def try_generate_hard_plus(_seed):
    ...
    if diff in ('expert', 'extreme', 'diabolic'):
        # Quality filters for extreme and diabolic: no bifurcation, >= 6 distinct techniques
        if diff in ('extreme', 'diabolic'):
            if 'bifurcation' in techniques:
                return None  # reject: must be solvable by pure logic
            if len(techniques) < 6:
                return None  # reject: must require at least 6 distinct techniques
        values = [puzzle[r*9:(r+1)*9] for r in range(9)]
